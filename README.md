...
| Hop-by-hop headers | `Connection`, headers named by `Connection`, `Keep-Alive`, `Proxy-Connection`, `TE`, `Trailer`, `Transfer-Encoding`, `Upgrade`, and proxy authentication headers are not forwarded. |
...
For 5xx responses, Rest Gate also leaves `Content-Length` generation to the server because telemetry can append a trace ID to the response body.

Form bodies are proxied without re-encoding. The complete incoming `Content-Type`, including multipart boundary parameters, is preserved. Rest Gate snapshots Falcon's one-pass request input before Sinatra parses form parameters, gives Sinatra a separate in-memory stream, and forwards the untouched snapshot. Rack multipart files are released after the response lifecycle.
