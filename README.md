...
Form bodies are proxied without re-encoding. The complete incoming `Content-Type`, including multipart boundary parameters, is preserved. A request-scoped rewindable input allows Sinatra to inspect form parameters without consuming the body before Rest Gate forwards it; its temporary resources and Rack multipart files are released after the request/response lifecycle.
