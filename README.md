...
Persistence runs synchronously after the upstream response is received, so normal filesystem latency contributes to request duration. Log serialization, file, and retention failures are fail-open: they are reported through the service logger but do not replace the upstream response.
...
| `Connection`, headers named by `Connection`, and other hop-by-hop headers such as `Keep-Alive`, `TE`, `Trailer`, `Transfer-Encoding`, and `Upgrade` | Not forwarded. |
| `Accept-Encoding` | Set to `identity` to avoid compressed-response inconsistencies. |

For responses, connection/framing headers and headers nominated by the upstream `Connection` header are removed. The server regenerates response framing after telemetry and compression middleware have produced the final body.
